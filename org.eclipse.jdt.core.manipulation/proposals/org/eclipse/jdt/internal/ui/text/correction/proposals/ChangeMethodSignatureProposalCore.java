...
public class ChangeMethodSignatureProposalCore extends LinkedCorrectionProposalCore {

	public interface ChangeDescription {
	}

	public static class SwapDescription implements ChangeMethodSignatureProposalCore.ChangeDescription {
		final int index;
	...
	public static class RemoveDescription implements ChangeMethodSignatureProposalCore.ChangeDescription {
	}

	static class ModifyDescription implements ChangeMethodSignatureProposalCore.ChangeDescription {
	...
	public static class EditDescription extends ChangeMethodSignatureProposalCore.ModifyDescription {
		String orginalName;
	...
	public static class InsertDescription extends ChangeMethodSignatureProposalCore.ModifyDescription {
	...
	private ChangeMethodSignatureProposalCore.ChangeDescription[] fParameterChanges;
	private ChangeMethodSignatureProposalCore.ChangeDescription[] fExceptionChanges;


	public ChangeMethodSignatureProposalCore(String label, ICompilationUnit targetCU, ASTNode invocationNode, IMethodBinding binding, ChangeMethodSignatureProposalCore.ChangeDescription[] paramChanges, ChangeMethodSignatureProposalCore.ChangeDescription[] exceptionChanges, int relevance) {
	...
	private void modifyParameters(ASTRewrite rewrite, MethodDeclaration methodDecl) {
		...
		for (int i= 0; i < fParameterChanges.length; i++) {
			ChangeMethodSignatureProposalCore.ChangeDescription curr= fParameterChanges[i];

			if (curr == null) {
				SingleVariableDeclaration oldParam= parameters.get(k);
				usedNames.add(oldParam.getName().getIdentifier());
				k++;
			} else if (curr instanceof ChangeMethodSignatureProposalCore.InsertDescription) {
				ChangeMethodSignatureProposalCore.InsertDescription desc= (ChangeMethodSignatureProposalCore.InsertDescription) curr;
			...
			} else if (curr instanceof ChangeMethodSignatureProposalCore.RemoveDescription) {
			...
			} else if (curr instanceof ChangeMethodSignatureProposalCore.EditDescription) {
				ChangeMethodSignatureProposalCore.EditDescription desc= (ChangeMethodSignatureProposalCore.EditDescription) curr;
			...
			} else if (curr instanceof ChangeMethodSignatureProposalCore.SwapDescription) {
				SingleVariableDeclaration decl1= parameters.get(k);
				SingleVariableDeclaration decl2= parameters.get(((ChangeMethodSignatureProposalCore.SwapDescription) curr).index);
	...
	private void fixupNames(ASTRewrite rewrite, ArrayList<String> usedNames) {
		...
		for (int i= 0; i < fParameterChanges.length; i++) {
			ChangeMethodSignatureProposalCore.ChangeDescription curr= fParameterChanges[i];
			if (curr instanceof ChangeMethodSignatureProposalCore.ModifyDescription) {
				ChangeMethodSignatureProposalCore.ModifyDescription desc= (ChangeMethodSignatureProposalCore.ModifyDescription) curr;
				...
				if (suggestedName != null) {
					...
					addLinkedPositionProposal(nameKey, favourite);
				}

				if (desc instanceof ChangeMethodSignatureProposalCore.EditDescription) {
					addLinkedPositionProposal(nameKey, ((ChangeMethodSignatureProposalCore.EditDescription)desc).orginalName);
				}
				...
				for (String sname : suggestedNames) {
					addLinkedPositionProposal(nameKey, sname);
				}
				if (favourite == null) {
	...
	private TagElement findParamTag(MethodDeclaration decl, SingleVariableDeclaration param) {
		...
		if (javadoc != null) {
			return JavadocTagsSubProcessorCore.findParamTag(javadoc, param.getName().getIdentifier());
		}
		return null;
	}
	...
	private TagElement insertParamTag(ListRewrite tagRewriter, List<SingleVariableDeclaration> parameters, int currentIndex, TagElement newTagElement) {
		...
		JavadocTagsSubProcessorCore.insertTag(tagRewriter, newTagElement, previousNames);
	...
	private void modifyExceptions(ASTRewrite rewrite, MethodDeclaration methodDecl) {
		...
		for (int i= 0; i < fExceptionChanges.length; i++) {
			ChangeMethodSignatureProposalCore.ChangeDescription curr= fExceptionChanges[i];
			...
			if (curr == null) {
				...
				Javadoc javadoc= methodDecl.getJavadoc();
				if (javadoc != null && JavadocTagsSubProcessorCore.findThrowsTag(javadoc, type) == null) {
					TagElement newTagElement= ast.newTagElement();
				...
				Type decl2= exceptions.get(((ChangeMethodSignatureProposalCore.SwapDescription) curr).index);
	...
	private TagElement findThrowsTag(MethodDeclaration decl, Type exception) {
		...
		if (javadoc != null) {
			...
			return JavadocTagsSubProcessorCore.findThrowsTag(javadoc, name);
		}
		return null;
	}
